 ...
 */
package co.yixiang.common.web.service.impl;
...
import co.yixiang.config.SystemConfig;
...
public abstract class BaseOpenServiceImpl<M extends BaseMapper<T>, T> extends ServiceImpl<M, T> implements BaseService<T> {
    ...
    protected void checkAuth(Object param) {
        Map<String,String> map = JSON.parseObject(JSON.toJSONString(param), Map.class);
        String signature = map.get("signature");
        map.remove("signature");
        if (!signature.equals(SignatureUtil.getSignature(map, SystemConfig.appKey))) {
