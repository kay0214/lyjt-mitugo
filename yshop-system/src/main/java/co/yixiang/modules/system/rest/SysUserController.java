...
public class SysUserController {
    ...
    @ApiOperation("查询当前有效店铺")
    @GetMapping(value = "/getStoreInfo")
    @PreAuthorize("@el.check('admin','user:list')")
    public ResponseEntity<Object> getStoreInfo() {
        List<YxStoreInfo> list = this.yxStoreInfoService.list(new QueryWrapper<YxStoreInfo>().lambda().eq(YxStoreInfo::getDelFlag, 0));
        if (null == list || list.size() <= 0) {
            return new ResponseEntity<>(new ArrayList<YxStoreInfo>(), HttpStatus.OK);
        }
