      ...
      <div class="myService">
        <div class="serviceList acea-row row-middle">
          <template v-for="(item, index) in MyMenus">
            <cell :title="item.name" is-link :icon="item.pic" @click="goPages(index)"/>
          </template>
        </div>
      </div>
    </div>
    <p style="text-align: center;margin-top: 0.2rem;height: 1rem">www.yixiang.co提供技术支持</p>
...
export default {
  ...
  methods: {
    ...
    goPages: function(index) {
      ...
      if (url === "/order/order_cancellation" && !this.userInfo.checkStatus) {
        return this.$dialog.toast({ mes: "您没有核销权限,请后台店员设置！！" });
      }
