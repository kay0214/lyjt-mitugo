<template>
  <div class="order-submission">
    <div class="allAddress" :style="system_store ? '' : 'padding-top: 0.2rem'">
      ...
      >
        <div class="addressCon" v-if="addressInfo.realName">
          ...
          <div>
            <span class="default font-color-red" v-if="addressInfo.isDefault"
              >[默认]</span
            >
      ...
      >
        <div class="addressCon">
          <div class="name">
            {{ storeItems.name || system_store.name }}
            <span class="phone">{{
              storeItems.phone || system_store.phone
            }}</span>
          </div>
          <div>
            {{ storeItems.address || system_store.address }}
          </div>
        </div>
        <div class="iconfont icon-jiantou"></div>
      </div>
              ...
              <label class="well-check">
                ...
                <span class="integral">
                  ...
                  <span
                    ...
                    v-if="
                      userInfo.integral > enableIntegralNum &&
                        enableIntegralNum > 0
                    "
                  >
                    {{ userInfo.integral || 0 }},可用{{ enableIntegralNum }}
                  </span>
                  <span class="num font-color-red" v-else>
                    {{ userInfo.integral || 0 }}
                  </span>
                </span>
              </label>
    ...
    <div class="moneyList">
      ...
      <div
        class="item acea-row row-between-wrapper"
        v-if="orderPrice.deductionPrice > 0"
      >
        <div>积分抵扣：</div>
...
export default {
  ...
  methods: {
    ...
    addressType: function(index) {
      ...
      console.log("index2:" + this.shipping_type);
    },
    computedPrice() {
    ...
    getCartInfo() {
      ...
      postOrderConfirm(cartIds)
        .then(res => {
          ...
          this.enableIntegral = res.data.enableIntegral;
          this.enableIntegralNum = res.data.enableIntegralNum;
          this.usableCoupon = res.data.usableCoupon || {};
          this.addressInfo = res.data.addressInfo || {};
          this.system_store = res.data.systemStore || {};
          this.store_self_mention = res.data.storeSelfMention;
          this.computedPrice();
        })
        .catch(() => {
    ...
    createOrder() {
      ...
      createOrder(this.orderGroupInfo.orderKey, {
        ...
        storeId: this.storeItems.id || this.system_store.id
      })
        .then(res => {
