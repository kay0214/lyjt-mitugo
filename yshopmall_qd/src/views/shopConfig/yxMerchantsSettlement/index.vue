...
<!--        <el-table-column type="selection" width="55" />-->
        ...
        <el-table-column v-if="columns.visible('status')" prop="status" label="状态" >
          <template slot-scope="scope">
            <span>
              <el-tag v-if="scope.row.status === 1" :type="'success'">{{ transferLabel(scope.row.status,statusList) }}</el-tag>
              <el-tag v-else-if="scope.row.status === 0" :type=" '' ">{{ transferLabel(scope.row.status,statusList) }}</el-tag>
